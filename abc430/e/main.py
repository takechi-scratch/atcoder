...
class RollingHash:
    ...
    def __getitem__(self, item):
        ...
        return self.get(item.start if item.start is not None else 0, item.stop if item.stop is not None else 0)


def solve(A: str, B: str):
    A_hash = RollingHash(A)
    ...
    for i in range(1, len(A)):
        if A_hash[:i] == B_hash[len(A) - i :] and A_hash[i:] == B_hash[: len(A) - i]:
            return i
